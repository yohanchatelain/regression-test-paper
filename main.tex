...
% statistical test
Our stability test results from multiple design choices. The use of a parametric test in Equation~\ref{eq:pval} simplifies calculations compared to non-parametric methods that typically require large sample sizes---in our case, more than $100$ repetitions per subject---to achieve an acceptable confidence level ($>0.95$). However, the computational cost for real data analysis tools such as \fmriprep is prohibitively high in terms of time and storage space \TG{sure but this data is also not required to run the test}. Working on other output types, such as regions of interest with the produced segmentations, could reduce the volume of elements to be processed and enable more complex analyses \TG{this won't reduce compute time}.
...
Nevertheless, our method is sufficiently precise to detect between-subject variability, even at higher levels of smoothing. The numerical perturbations introduced by Fuzzy-libmath only affect a small portion of the calculations performed in \fmriprep.
...
%   Extends instrumentation, explain benefits and challenges
...
Subjects were chosen to reflect diverse acquisition qualities, resolutions, ages, and genders.
