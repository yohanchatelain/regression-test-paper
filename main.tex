...
\TODO{Yohan: I added subsections to make reading easier. They would be removed then. News subsubsection indicate the rewrite}
...
%   non-parametric have better fitting
%   
The use of a parametric test in Equation~\ref{eq:pval} simplifies calculations compared to non-parametric methods that typically require large sample sizes --- in our case, more than $100$ repetitions per subject --- to achieve an acceptable confidence level ($>0.95$).
%   Non-parametric tests require a larger sample size
%   Perturbed sample acquisition is prohibitive (time and space)

However, the computational cost for real data analysis tools such as \fmriprep is prohibitively high in terms of time and storage space \TG{sure but this data is also not required to run the test}.
% Reducing the number of multiple tests (depending on the number of voxels) allows performing more complex statistical analyses such as correlations
Working on other output types, such as regions of interest with the produced segmentations, could reduce the volume of elements to be processed and enable more complex analyses \TG{this won't reduce compute time}.
...
% perturbation
We used Random Rounding to sample the null distribution of the results as this method can be applied to data analyses regardless of the type of input and output data involved.

%   RR comparable to RS
%   RR made no assumptions on the underlying code (except using libm functions for fuzzy-libmath)
This is an independent method to evaluate the numerical quality of neuroimaging pipelines since it generates variability comparable to random-seed.

The numerical perturbations introduced by Fuzzy-libmath only affect a small portion of the calculations performed in \fmriprep.
%   RR could miss calculations not involving libm
Although it reproduces the measured variability by varying the random seed, it does not stimulate calculations performed elsewhere in advancing neuroimaging.
%   Extend instrumentation, explain benefits and challenges
Extending the test to other application domains where the libmath has little effect on stability~\cite{pepe2022numerical} will require perturbing other computation libraries.
%   RR gives similar results as RS (add boxplots + statistical tests?)
Furthermore, we demonstrated that the Fuzzy-libmath perturbations type yielded comparable numerical quality results to those obtained with random seeds, highlighting the ability of the stochastic arithmetic approach to accurately simulate numerical instabilities.
...
% Specificity requires smoothing

%   Tradeoff between anatomical VS pure numerical point of view
The solutions obtained for the smoothing kernel size may not be anatomically satisfactory for neuroimaging researchers (FWHM $\geq 15$) in terms of anatomical accuracy, but they are valid from a software engineering perspective, as they allow for the detection of significant changes between different versions of \fmriprep.
%   Duplicated with previous paragraph (to remove?)
Although our methods may not be currently applicable for neuroimaging research stricto sensu, they serve as a stability test for identifying noticeable differences between computed outputs \TG{not sure about the point being made here}.
...
%   smoothing makes voxel distribution more normal
The smoothing size tended to affect the performance of the stability test, as it rendered the distribution of voxels more normal.
%   detect between-subject variability
Nevertheless, our method is sufficiently specific to detect between-subject variability, even at higher levels of smoothing.
...
%   Numerical variability varies across subjects
The numerical stability of the results varied importantly across subjects.
%   Explain data selection
%   Good hyperparameters are dataset dependent
Although our analysis did not precisely determine which factors specifically influenced the stability test, we observed that certain smoothing sizes were more appropriate than others.
%   Not sure about the logic of the sentence. What do we mean?
From a software engineering perspective, our stability test applies to a carefully selected database of test subjects, as we were able to identify valid configurations $\alpha$ and FWHM for which the test passed our sanity checks.
%   Valid configurations are dataset dependent but it exists at least one per dataset
The identified configurations varied across subjects, depending on the quality of the acquisition data, indicating that our test is robust to heterogeneously quality data.
...
%   Bonferroni is conservative so it tends to be less sensitive (not detecting true positive (image outside of reference distribution))
%   Since it is conservative, our test should be very permissive 
%   But it is not the case. 
%   So?
%   
The Bonferroni correction, although conservative, fails to capture observations from reference distributions, as demonstrated by LOO (Figure~\ref{fig:loo_bonferroni}) and IEEE (Figure~\ref{fig:ieee-check}) checks \TG{why is it a failure?}.
%   
This could be attributed to two factors.
%   limitation of Bonferroni for dependent tests.
%   first sentence is a consequence of the second -> reformulate
%   What do we want to say?
Firstly, voxels in neuroimaging data are not spatially independent, and secondly, perturbed samples used in the analysis are not independent due to the optimization problems involved in pre-processing, such as spatial normalization and bias field correction.
%   This is why we're doing this statistical test and numerical optimization more broadly
These optimization problems do not have analytical solutions and require numerical methods for addressing them.
%   These optimizations problems are sensitive to tiny perturbations
%   Perturbations will produce different outputs
%   Outputs are valid from a neuroimaging point of view (explain the valid term)
However, due to the inherent complexity of these problems (\cite{clarenz2006computational}), even small perturbations in the input data can result in substantially different but equally valid computed solutions.
%   Perturbations will produce different outputs
%   Outputs are valid from a neuroimaging point of view
As a consequence, minimal variations in calculations introduced by Fuzzy-libmath can lead to significantly different geometric solutions, resulting in regions of high instability within anatomical brain images, particularly at white/grey matter borders (Figure~\ref{fig:uncertainty-maps}).
%   Limitation of the voxel-wise analysis.
%   Do not take into account local information (voxel neighborhood)
And so when working voxel-wise, the obtained brain geometries can differ significantly at the voxel level, transitioning from intensity levels characteristic of gray matter to white matter, and vice versa.
%   Limitation of the sample size
%   Explain that perturbed observation is costly to acquire
%   We possibly sample a large space
Furthermore, subsampling the solution space of optimization problems may lead to the omission of possible white matter regions where only gray matter was observed, and vice versa.

%   voxel-wise analysis does not take into account local information around each voxel.
However, voxel-wise analysis prevented the prediction of excessively large non-local variations.
Incorporating neighborhood information could improve learning, such as using more general deep learning methods like VoxelMorph~\cite{balakrishnan2019voxelmorph}. \TG{I don't understand this paragraph}.
...
%   The test can be applied broadly to any scientific results
...
%   The case of functional neuroimaging.
In future work, we plan to extend our test to other types of data, such as those used in functional neuroimaging.
%   What about relative differences rather than absolute magnitudes
...
%   Focus on specific axes rather than considering all axes with the same 
...
Looking forward, we plan to expand our methodology to include other data types and test its performance under various statistical hypotheses. Specifically, we aim to extend our test to functional neuroimaging data, taking into account temporal dependencies and focusing on the relevance of activation potential rather than the absolute magnitude.
