...
\title{A numerical precision approach to results stability tests and its application to neuroimaging}
...
Data analyses can produce different results depending on
the hardware and software conditions in which they are executed~\cite{glatard2015reproducibility, bhagwat2021understanding}, which has important repercussions
in a number of disciplines. This paper investigates results stability tests whereby the outcome of a data
analysis is asserted to remain within acceptable variation bounds of a reference result. The main
difficulty associated with the development of such results stability tests lies in the determination of 
acceptable bounds of variation. We propose to define such bounds from the numerical variability of the results, that is,
from the variability inherent to numerical computations. Numerical variability explains a substantial fraction of
results variability across execution environments when complex data analyses are involved. 

% neuroimaging
Although the testing methodology developed in the present paper applies to data analyses widely, we focus on 
neuroimaging analyses as our motivating use case. For several decades, the neuroimaging community has 
developed advanced software tools enabling researchers to study the human brain with unprecedented detail and precision.
A significant obstacle in assessing neuroimaging software stems from the lack of a well-established reference (a.k.a. ground truth) for 
the results. Methods such as simulations~\cite{simulation}, phantoms~\cite{phantom}, and ex-vivo data~\cite{exvivo} have limitations and do not provide a comprehensive solution for defining acceptable variation bounds.

% fmriprep
The present paper focuses primarily on the \fmriprep software~\cite{esteban2019fmriprep}, a tool to process structural and 
functional MRI data. In the past years, \fmriprep developers initiated long-term support (LTS) releases to guarantee the stability 
of results over time, a critical issue for neuroscience studies that commonly span multiple years. 
In this context, results stability tests are particularly important, due to inevitable security updates and bug fixes in the main software or in its dependencies.
\fmriprep is an integrated data analysis pipeline for structural and functional MRI pre-processing, which includes image processing operations typically 
encountered in MRI analysis. We focus primarily on the pre-processing of structural MRIs as implemented in \fmriprep, which includes intensity non-uniformity corrections, skull stripping, and spatial normalization to a brain template. Spatial normalization is a critical step of MRI analysis whereby images acquired on individual subjects are mapped to a template brain through a process called image registration. 

% numerical precision
We construct results stability tests by defining acceptable variation bounds from the numerical variability resulting from the use of finite-precision arithmetic in calculations.
Such errors arise when changes occur in the computational environment, including the computing architecture, the operating system, or the version of a dependent library for a given application. A well-established technique for estimating this uncertainty is Monte Carlo Arithmetic (MCA), a member of the family of stochastic arithmetics. Stochastic arithmetic models floating-point calculation errors with random variables, transforming the challenging task of symbolic round-off error analysis into a more tractable statistical sampling problem. Importantly, MCA is not dependent on the specificities of the application domain, rendering it applicable across a diverse range of cases and scenarios. Notably, MCA has been successfully utilized in neuroimaging to simulate numerical variability resulting from changes in operating systems~\cite{salari2021accurate} and to evaluate the reliability of brain networks~\cite{kiar2021numerical}.
...
of the application, and its sensitivity through the rejection of results produced by a different subject than the original result.
...
\begin{figure}
    ...
    \caption{Ratios of successful stability tests for the intra-subject IEEE check for RR (top) and RS (bottom). Low ratios for low FHWM sizes suggest a lack of specificity in these cases. The inter-subject tests (not displayed in the figure) consistently fail for all $\alpha$ and FWHM combinations, demonstrating excellent sensitivity.
    }
