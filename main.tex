...
\begin{equation}
    \label{eq:bonferroni}
    \forall i < v, \quad p_i \geq \frac{\alpha}{v}.
\end{equation}
...
\begin{figure}
    ...
    \caption{IEEE check for RR (top) and RS (bottom). Ratios of passed stability tests using the IEEE sample of the reference subject. Low ratios for low FHWM sizes suggest a lack of specificity in these cases. However, the stability test correctly rejects IEEE samples coming from another subject, demonstrating good sensitivity.
    }
...
\begin{figure}
    ...
    \caption{Corrupted template check for RR (top) and RS (bottom) modes. Each row represent a percentage of voxels set to zero. Stability test detects the noise introduction }
