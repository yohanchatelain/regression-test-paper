...
% context
The ubiquitous application of computational approaches across disciplines demands the consideration of data processing as part of the measurement device and sets computational stability as one of the preconditions or assumptions of the data analysis.
Indeed, repeated computational processing and analyses of the same data can produce different results depending on the hardware and software conditions in which they are executed~\cite{gronenschild2012effects}.
Computational stability is particularly under the spotlight in neuroimaging, a field where software tools (e.g., \emph{FSL}~\cite{jenkinson2012fsl}, \emph{FreeSurfer}~\cite{fischl2012freesurfer}, or {\emph{ANTs}~\cite{avants2009advanced}) % suggestion: cite also AFNI
have built up into sophisticated instruments enabling researchers to study the human brain with unprecedented detail and precision.
Unsurprisingly, the reproducibility and repeatability of neuroimaging results have been a focus of research for decades, yielding high-value recommendations and defining best practices applicable across computational sciences~\cite{tustison2013instrumentation}. One thoroughly examined application is brain surface reconstruction and quantification of derived features such as cortical thickness, which have been shown significantly different across software packages~\cite{mikhael2019controlled} executed on the same brains. Glatard et al.~\cite{glatard2015reproducibility} demonstrated operative system effects on measured cortical thickness within several software packages and package versions.
% neuroimaging
...
Previously, computational stability has been addressed through random rounding (RR).
...
with a virtual precision of 53 bits and single-precision numbers with a virtual precision of 24 bits,
...
Over the years, extensive research has focused on ensuring consistency and accuracy of neuroimaging results, leading to best practices recommendations~\cite{tustison2013instrumentation}. However, 

A key area of study is brain surface reconstruction, specifically analyzing attributes like cortical thickness. Interestingly, the results vary significantly across different software, even when they're working with the same brain data. Evidence of this is highlighted in the work of Glatard and colleagues, who showed how different operating systems can influence the measurements of cortical thickness using various software packages.

The present paper focuses primarily on the fMRIPrep software~\cite{esteban2019fmriprep}, a tool to pre-process Magnetic Resonance Imaging (MRI) data as a pre-requisite of any further analysis. fMRIPrep is an integrated data analysis pipeline for structural and functional MRI pre-processing. We focus on the pre-processing of structural MRI, which includes intensity non-uniformity correction, skull stripping, and spatial normalization to a brain template. fMRIPrep developers recently initiated long-term support (LTS) releases to guarantee results stability over multiple years, which motivated the development of the results stability tests presented in this paper.
...
\begin{equation}
  \begin{split}
    \delta(n, \alpha_s, p_s) =& \frac{1}{2} \log_2 \left( \frac{n-1}{\chi^2_{1-\alpha_s/2}} \right) +
    \log_2 \left( \Phi^{-1} \left( \frac{p_s+1}{2} \right) \right)
  \end{split}
\end{equation}
